...
export default class DefeatPage extends Phaser.Scene {
	...
	editorCreate(): void {
		...
		// bgClouds
		const bgClouds = this.add.tileSprite(920, 284, 1029, 242, "Purple_Green_Pixel_Illustration_Game_Presentation__2_-removebg-preview");
		...
		// bgClouds_1
		const bgClouds_1 = this.add.tileSprite(952, 1068, 1029, 242, "Purple_Green_Pixel_Illustration_Game_Presentation__2_-removebg-preview");
	...
	startAnimationSequence() {
		...
		this.tweens.add({ targets: this.platform_1, y: 608, alpha: 1, duration: 2000, ease: 'Power2' });
		this.time.delayedCall(500, () => {
			this.tweens.add({ targets: this.platform_2, y: 560, alpha: 1, duration: 2000, ease: 'Power2' });
		});
		this.time.delayedCall(1000, () => {
			this.tweens.add({ targets: this.platform_3, y: 608, alpha: 1, duration: 2000, ease: 'Power2' });
		});

		// Defeat Text & Banner fade-in
		this.tweens.add({ targets: this.defeatText, alpha: 1, duration: 2000, ease: 'Linear' });
		...
		this.time.delayedCall(8000, () => {
			...
			this.time.delayedCall(2000, () => this.scene.start("LandingPage"));
		});
	}
	/* END-USER-CODE */
}
