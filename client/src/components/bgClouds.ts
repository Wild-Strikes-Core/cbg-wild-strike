export default class bgClouds extends Phaser.GameObjects.Image {
	...
	constructor(scene: Phaser.Scene, x?: number, y?: number, texture?: string, frame?: number | string) {
		...
		if (!scene.textures.exists(textureToUse)) {
			console.warn(`Texture ${textureToUse} not found. Using a placeholder.`);
			...
			super(scene, x ?? scene.scale.width + 200, centerY, 'cloud_placeholder');
		} else {
			super(scene, x ?? scene.scale.width + 200, centerY, textureToUse, frame);
		}
